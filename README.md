...
- `0.0` ボタン：エンジン出力を0にする
- `+1.0` ボタン：最大出力で右へ進む
- Reset：ゲームを最初からやり直す。0以外の入力が与えられるまで物理時間は進まない
- キーボード補助操作
  - `←`：-1.0
  - `→`：+1.0
  - `Space`：0.0
...
ゴールまたはステップ上限に達した直近5回の記録は、
スライダー下部のプレイ画面内に表示されます。
記録はページを開いている間だけ保持され、再読み込みすると消えます。
Clearボタンでも削除できます。
...
PCでは2カラム表示です。左にプレイ画面・状態表示・スライダー、右に直近の記録とヒントを配置します。スマートフォンでは1カラムの縦並びに自動で切り替わります。
